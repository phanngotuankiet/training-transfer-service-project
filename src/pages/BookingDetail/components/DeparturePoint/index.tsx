...
const DeparturePoint = ({ startLocation, endLocation, status }) => {
  ...
  >
    {status === 'Completed' ? 'Đã hoàn thành' : status === 'Pending' ? 'Đang chờ xác nhận' : status === 'Cancelled' ? 'Đã huỷ' : status === 'Confirmed' && 'Tài xế đang tới'}
  </span>

  return (
    <div>
      <p className="font-bold text-sm flex space-x-2 items-center"><span>Điểm khởi hành</span> {showStatusHere}</p>
